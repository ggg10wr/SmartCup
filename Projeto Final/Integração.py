...
def alertaTelegram():
    ...
    for usuario in usuarios_atuais :
        ...
        if (ml_bebidas < litros_objetivo):
            if streak == 0 :
                dados1 = {"chat_id": id_da_conversa, "text": f" 💧Ei, hidratado ou desidratado? 💧\nVocê já bebeu {ml_bebidas}L de água hoje.\nMeta do dia: {litros_objetivo}L.\nFalta só {diferenca}L pra virar um aquário ambulante! 🐠🥤"}
            else :
                dados1 = {"chat_id": id_da_conversa, "text": f" 💧Ei, hidratado ou desidratado? 💧\nVocê já bebeu {ml_bebidas}L de água hoje.\nMeta do dia: {litros_objetivo}L.\nFalta só {diferenca}L pra virar um aquário ambulante! 🐠🥤\n🔥 Bora manter a chama viva!🔥"}
                
        else:
            if streak == 0 :
                dados1 = {"chat_id": id_da_conversa, "text": f"🎉 Parabéns! Você já bebeu {ml_bebidas}mL de água hoje — missão cumprida!\nMeta do dia: {litros_objetivo}L."}
            elif streak == 1 :
...
class JanelaInfo(tk.Tk):
    ...
    def cadastrar_usuario(self, event):
        ...
        if not(NomeUsuario.strip()) or not(MetaUsuario.strip()) or not(IdChatUsuario.strip()) or not(PesoUsuario.strip()) or not(TagUsuario.strip()) or not(UrlUsuario.strip()) or not(CorUsuario.strip()):
            ...
            usuario = {"nome": f"{NomeUsuario}", "cor": f"{numero_cor}", "meta": f"{MetaUsuario}", "idchats": f"{IdChatUsuario}", "tara": f"{PesoUsuario}", "idtag": f"{TagUsuario}", "utimo_peso": 0, "streak": 0, "dias": [datetime.now()], "volume": [0], "urlwidget": f"{UrlUsuario}"}
            colecao_de_usuarios.insert_one(usuario)
            self.destroy()
            app = JanelaRecepcao()
            app.mainloop()
    ...
    def ler_tag(self, event):
        ...
        thread = threading.Thread(target=self.serial_tag)
        thread.daemon = True
        thread.start()
    ...
    def medir_peso(self, event):
        ...
        thread_peso = threading.Thread(target=self.serial_peso)
...
class JanelaEdicao (tk.Tk):
    def __init__(self, indice):
        ...
        self.Meta = tk.Entry(self, width=40)
        self.Meta.insert(0, self.usuario_editado["meta"])
        self.Meta.place(x=200,y=120)
    ...
    def modificar_usuario(self, event):
        ...
        if not(NomeUsuario.strip()) or not(MetaUsuario.strip()) or not(IdChatUsuario.strip()) or not(PesoUsuario.strip()) or not(TagUsuario.strip()) or not(UrlUsuario.strip()) or not(CorUsuario.strip()):
            ...
            colecao_de_usuarios.delete_one(dado_delete) 
            usuario = {"nome": f"{NomeUsuario}", "cor": f"{numero_cor}", "meta": f"{MetaUsuario}", "idchats": f"{IdChatUsuario}", "tara": f"{PesoUsuario}", "idtag": f"{TagUsuario}", "utimo_peso": 0, "streak": 0, "dias": vecteur_temps, "volume": vecteur_volume, "urlwidget": f"{UrlUsuario}" }
            colecao_de_usuarios.insert_one(usuario)
            self.destroy()
            app = JanelaRecepcao()
            app.mainloop() 
...
thread_telegram = threading.Thread(target=verificacao_temporal_alerta)	#thread do telegram
thread_telegram.daemon = True
thread_telegram.start()

#semaforo = threading.Semaphore(1)
thread = threading.Thread(target=verificacao_peso)
thread.daemon = True
thread.start()

thread = Thread(target=verificacao_temporal_alerta)
thread.daemon = True
thread.start()
